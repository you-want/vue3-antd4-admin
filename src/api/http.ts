  ...
  public constructor(config: AxiosRequestConfig) {
    ...
    this.service.interceptors.response.use(
      ...
      async (error: AxiosError) => {
        ...
        if (error.message.indexOf("timeout") !== -1) messageApi.error("请求超时！请您稍后重试");
        if (error.message.indexOf("Network Error") !== -1) messageApi.error("网络错误！请您稍后重试");
